...
<div class="form-group {{ $errors->has('username') ? 'has-error' : '' }}">   
 ...
 <label class="col-sm-2 control-label" for="inputError">{{$formselect->formcheckbox_name}}</label> @else
 <label class="col-sm-2 control-label">{{$formselect->formcheckbox_name}}</label> @endif
<div class="col-sm-8"> 
<input type="text" name="formselect_name[]"  placeholder="{{$formselect->formcheckbox_name}}" class="form-control"   value="{{$formselect->formcheckbox_name}}"   > 
</div> 
 <div class="col-sm-2"  > <a    href="{{env('APP_URL')}}/superadmin/viewsforms/editcheckboxdelet/{{$formselect->formcheckbox_id}}"        > <span class="label label-danger">حذف</span></a></div>	
</div> 
<div class="line-dashed"></div>
