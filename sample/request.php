...
$parameters = array(
    ...
    'Amount' 			=> $price,
    'OrderId' 			=> $idorder,
    'CallBackUrl' 		=> 'https://azmoonpte.com/servicepay/sample/verify.php?id='.$idorder,
    'AdditionalData' 	=> ''
);
...
if ($token > 0 && $status==0) {
    header('location:https://servicepay.azmoonpte.com/user/testcurl/'.$idorder.'/'.$token.'/'.$status);
    exit;
} else {
